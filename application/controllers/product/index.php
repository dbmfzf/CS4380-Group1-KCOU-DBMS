...
class Index extends CI_Controller {
	...
	public function index()
	{
		...
		$most_query = $this->db->query("SELECT S.path as song_path, S.title as song_title FROM Search_song S1, Song S WHERE S1.sid = S.sid AND S1.uid = '".$uid."' group by S1.sid having count(*) = (SELECT MAX(S2.sid) FROM Search_song S2 WHERE S2.uid = '".$uid."' group by S2.sid ORDER BY date_time limit 1) order by S1.date_time desc limit 1");
